...
>
>**触发方式:自动触发或手动触发**
>自动触发: 在redis.conf配置中配置save参数(表示m秒内数据集存在n次修改时,自动触发bgsave)
>```
> save 900 1    # 表示900秒内如果至少有1个key值变化，就写一份新的RDB文件
> save 300 10   # 表示300秒内如果至少有10个key值发生变化，就写一份新的RDB文件
> save 60 10000 # 表示60秒内如果至少有10000个key值发生变化，就写一份新的RDB文件
> ```
>手动触发: 手动生成新的RDB文件，执行Redis的save、bgsave命令
...
>redis把操作指令追加到AOF文件的这个过程,并不是直接写到AOF文件中,而是先写到操作系统缓存中,这个内存缓存是由操作系统内核分配的,然后操作系统内核会异步把缓存中的redis操作指令刷写到AOF文件中，假如缓存中的redis指令还没刷写到AOF文件中就宕机了,不过
>glibc函数库提供了fsync(),该函数可以指定文件的内容强制从缓存中刷写到磁盘上,fsync的操作周期对redis的性能有很大影响
>AOF日志文件会越来越大,redis重启从AOF文件恢复执行指令将会非常耗时,所以需要对AOF文件"瘦身",称为AOF重写(rewrite)
>AOF Rewrite原理:
>主进程会fork一个子进程,对当前内存的数据进行遍历并转换为redis的指令,将指令序列化到新的AOF日志中,序列化的过程中收到的操作指令追加到新的AOF日志文件中,新的AOF文件替换就的AOF文件
>
>默认是不开启的
>
>**相关配置**
...
##### 混合持久化
RDB恢复内存状态,可能会丢失大量数据,AOF来恢复性能较差,在redis4.x后混合持久化（RDB+AOF）对重写的优化，4.0版本的混合持久化默认是关闭的，可以通过以下的配置开启混合持久化
RDB文件内容+增量的AOF文件(RDB持久化开始到结束的这段时间的增量,从开始写RDB文件开始,新接收的指令放到AOF文件中)
>
>```
